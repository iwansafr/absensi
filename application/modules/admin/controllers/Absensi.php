...
class Absensi extends CI_Controller
{
	...
	public function rekap($k_id = 0)
	{
		if(!empty($k_id))
		{
			...
			if(empty($_GET['excel'])){
				...
				$this->load->view('admin/absensi/rekap',['data'=>$data,'karyawan'=>$karyawan,'month'=>$month,'year'=>$year,'instansi'=>$instansi,'masuk'=>$masuk]);
			}
		}
	}
	...
	public function qr()
	{
		...
		if (!empty($data)) {
			...
			$data['jadwal'] = $jam_today['jadwal'];
			$exist = $this->db->get_where('absensi',['karyawan_id'=>$g_id,'status'=>$data['status'], 'CAST(waktu AS date)=' => date('Y-m-d')])->row_array();
			$allowed = true;
			if(!empty($exist['id'])){
